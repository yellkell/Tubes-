 ...
 * post-processing (fragile/expensive in stereo WebXR, and in passthrough
 * a post pass would have to composite over the camera feed anyway).
 * Everything renders additively with depthWrite off, so hot cores bleed
 * soft halos over the real room. Carried over from RAVE RAID.
 */
 ...
 * confetti look this shop is not going for.
 */
 ...
 * most of the difference between "a glowing cone" and "a shaft of sun
 * through haze": a flat-opacity cone ends in mid-air as a hard solid,
 * while a ramped one simply runs out of light.
 *
 * `sourceAtTop` picks which V end is the bright one: the socket shaft
 * hangs its source at V=1 (the wall), beams grow from V=0.
 */
